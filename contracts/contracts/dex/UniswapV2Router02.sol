...
import "./interfaces/IWECY.sol";
import "./libraries/UniswapV2Library.sol";

/// @title UniswapV2Router02 - User-facing router for swaps and liquidity
contract UniswapV2Router02 {
    address public immutable factory;
    address public immutable WETH; // WECY on EnergyChain
    ...
    function addLiquidityETH(
        ...
        IWECY(WETH).deposit{value: amountETH}();
        assert(IWECY(WETH).transfer(pair, amountETH));
        liquidity = IUniswapV2Pair(pair).mint(to);
    ...
    function swapExactETHForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        IWECY(WETH).deposit{value: amounts[0]}();
        assert(IWECY(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactETH(
        ...
        IWECY(WETH).withdraw(amounts[amounts.length - 1]);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        IWECY(WETH).deposit{value: amountIn}();
        assert(IWECY(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(IERC20(path[path.length - 1]).balanceOf(to) - balanceBefore >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 amountOut = IWECY(WETH).balanceOf(address(this));
        require(amountOut >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
        IWECY(WETH).withdraw(amountOut);

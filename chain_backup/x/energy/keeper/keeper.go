...
// GenerateID returns a monotonically increasing ID string scoped to the store.
...
func (k Keeper) SubmitEnergyData(ctx sdk.Context, data types.EnergyData) {
	...
	store.Set(types.GetEnergyByTypeKey(data.DataType, data.ID), []byte(data.ID))
...
func (k Keeper) GetEnergyDataByType(ctx sdk.Context, dataType types.EnergyDataType) []types.EnergyData {
	store := ctx.KVStore(k.storeKey)
	prefix := types.GetEnergyByTypePrefixKey(dataType)
...
// IsAllowedSubmitter returns true if the address is permitted to submit data.
// An empty AllowedSubmitters list means anyone is allowed.

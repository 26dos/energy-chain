...
func (gs GenesisState) Validate() error {
	...
	for i, d := range gs.DataRecords {
		...
		ids[d.ID] = true
		if !IsValidDataType(d.DataType) {
			return fmt.Errorf("data record %s has invalid data type: %d", d.ID, d.DataType)
		}
		if d.DataHash == "" {
	...
	for i, b := range gs.Batches {
		...
		batchIDs[b.ID] = true
		if b.Submitter == "" {
			return fmt.Errorf("batch %s has empty submitter", b.ID)
		}
		if b.MerkleRoot == "" {
			return fmt.Errorf("batch %s has empty merkle root", b.ID)
		}
	}
